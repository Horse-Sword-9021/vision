...
def picture_show(choose, window_name, p_name):
    def cv_show(window_name, p_name):
        cv.imread()
        methods_imread = [cv.IMREAD_COLOR, cv.IMREAD_GRAYSCALE]
        # cv2.imread(p_name)#bgr,0~255
        cv.imshow(window_name, p_name)
    ...
    def plt_show(window_name, p_name):
        # plt.imread(p_name)
        plt.imshow(p_name, 'gray')
...
def face_detection_demo():
    ...
    cap = cv.VideoCapture('imgs/face.mp4')
    while True:
        ret, frame = cap.read()
        h, w, c = frame.shape
        if ret is not True:
